...
# --- Provider Gemini (di-load hanya jika dipakai) ---
gemini_client = None
types = None
...
def _get_ollama():
    ...
    if _ollama_client is None:
        ...
        _ollama_client = AsyncOpenAI(base_url=config.OLLAMA_BASE_URL + "/v1", api_key="ollama")
...
def _student_context(student_nim: str) -> str:
    """Instruksi tambahan ketika pengguna adalah mahasiswa yang login (auto-NIM + privasi)."""
    return (
        "\n\n# KONTEKS PENGGUNA (MAHASISWA LOGIN)\n"
        f"Pengguna saat ini adalah mahasiswa dengan NIM {student_nim}. "
        f"Jika ia bertanya tentang dirinya sendiri (mis. 'SKS saya', 'IPK saya', 'nilai saya', "
        f"'jadwal saya'), gunakan NIM {student_nim} pada query tanpa perlu menanyakannya lagi. "
        "ATURAN PRIVASI WAJIB: Pengguna HANYA boleh melihat datanya sendiri. Jika ia meminta data "
        f"mahasiswa dengan NIM selain {student_nim} (atau menyebut nama orang lain), TOLAK dengan sopan "
        "dan jangan jalankan query untuk data mahasiswa lain."
    )


def build_system_instruction(prompt: str, student_nim: str = None) -> str:
    """
    Gabungkan SYSTEM_INSTRUCTION dasar dengan konteks hasil RAG dan (opsional)
    konteks mahasiswa yang sedang login.
    """
    context_block = rag_service.build_context_block(prompt, k=4)

    instruction = SYSTEM_INSTRUCTION
    if context_block:
        instruction += (
            "\n\n# KONTEKS DOKUMEN KEBIJAKAN (RAG)\n"
            "Gunakan informasi di bawah ini untuk menjawab pertanyaan seputar kebijakan/ketentuan "
            "(misalnya UKT, KKN, aturan SKS, syarat kelulusan). Jika jawaban ada di konteks ini, "
            "jawab berdasarkan konteks dan sebutkan bahwa info berasal dari dokumen kebijakan kampus. "
            "Jika pertanyaan butuh data mahasiswa spesifik (NIM, KRS, nilai), tetap gunakan tool "
            "'execute_academic_query'. Jika informasi tidak ada di konteks maupun database, katakan "
            "terus terang bahwa Anda belum memiliki informasinya, JANGAN mengarang.\n\n"
            "----- AWAL KONTEKS -----\n"
            + context_block
            + "\n----- AKHIR KONTEKS -----\n"
        )
    ...
    if student_nim:
        instruction += _student_context(student_nim)

    return instruction


# Skema tool dalam format OpenAI/Ollama
OLLAMA_TOOLS = [{
    "type": "function",
    "function": {
        "name": "execute_academic_query",
        "description": (
            "Jalankan query SQL SELECT (read-only) ke database akademik untuk mengambil "
            "data mahasiswa, prodi, fakultas, KRS, dan mata kuliah."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "sql_query": {"type": "string", "description": "Query SQL SELECT yang aman."}
            },
            "required": ["sql_query"],
        },
    },
...
async def _ollama_generate(prompt: str, system_instruction: str) -> str:
    """Jalur Ollama: loop function-calling gaya OpenAI (kita eksekusi tool manual)."""
    client = _get_ollama()
    messages = [
    ...
    try:
        for _ in range(5):  # batasi langkah agar tidak loop tak terbatas
            resp = await client.chat.completions.create(
                model=LLM_MODEL, messages=messages, tools=OLLAMA_TOOLS, temperature=0.3,
            )
            ...
            if msg.tool_calls:
                ...
                for tc in msg.tool_calls:
                    ...
                    try:
                        ...
                        pass
                    if tc.function.name == "execute_academic_query":
                        result = execute_academic_query(args.get("sql_query", ""))
                    else:
                        result = "Tool tidak dikenal."
                    messages.append({"role": "tool", "tool_call_id": tc.id, "content": str(result)})
                continue
            return msg.content or ""
        return "Maaf, terlalu banyak langkah pemanggilan tool. Coba persempit pertanyaan Anda."
    except Exception as e:
        return f"Error dari Ollama Assistant: {str(e)}"


async def generate_response_async(prompt: str, student_nim: str = None) -> str:
    # RAG + konteks mahasiswa: bangun system instruction (dipakai kedua provider)
    system_instruction = build_system_instruction(prompt, student_nim=student_nim)

    if LLM_PROVIDER == "ollama":
        return await _ollama_generate(prompt, system_instruction)
