...
def reset_rag_table() -> dict:
    """
    Drop & buat ulang tabel rag_chunks dengan dimensi vektor terkini (config.EMBEDDING_DIM).
    WAJIB dipanggil bila mengganti model embedding ke dimensi berbeda (mis. 768 -> 1024),
    lalu lakukan ingest ulang. Vektor lama tidak kompatibel dengan model baru.
    """
    from app.database import engine
    from app.models.rag_document import RagChunk
    try:
        RagChunk.__table__.drop(engine, checkfirst=True)
        RagChunk.__table__.create(engine, checkfirst=True)
        return {"status": "ok", "message": "Tabel rag_chunks dibuat ulang. Lakukan /rag/ingest sekarang."}
